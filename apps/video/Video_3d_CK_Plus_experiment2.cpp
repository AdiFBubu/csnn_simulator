...
//        Experiment<TrainingSparseExecution> experiment(argc, argv, _dataset);
        ...
        //experiment.push<process::PercentileScaling>(0.95);
        ...
        // experiment.push<process::SumPooling>(8, 8);         // spatial
        // experiment.push<process::TemporalPooling>(3);        // temporal


        // auto &conv2 = experiment.push<layer::Convolution3D>(
        //        5, 5, 3, 16, "", 2, 2, tmp_stride);
        ...
        //conv2.parameter<uint32_t>("epoch").set(sampling_size * 2);
        //conv2.parameter<float>("annealing").set(0.95f);
        //conv2.parameter<float>("min_th").set(0.05f);
        //conv2.parameter<float>("t_obj").set(0.75);
        //conv2.parameter<float>("lr_th").set(0.2);
        //conv2.parameter<Tensor<float>>("w").distribution<distribution::Uniform>(0.0, 1.0);
        //conv2.parameter<Tensor<float>>("th").distribution<distribution::Gaussian>(1.5, 0.1);
        //conv2.parameter<STDP>("stdp").set<stdp::Biological>(0.05, 0.1f);
        //conv2.parameter<layer::ISampler3D>("sampler").set<layer::Sampler3DFacialPerVideo>();

        //auto &conv2_out = experiment.output<TimeObjectiveOutput>(conv2, 0.75, 0);
        ...
        //layer::ISampler3D* sampler_ptr2 = conv2.get_sampler();
        ...
        for (auto& seq : training_sequences) {
            ...
            try {
                ...
                //if (sampler_ptr2 != nullptr) {
                //    sampler_ptr2->add_sequence_bboxes(seq.bboxes);
                //}
