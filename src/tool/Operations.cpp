 ...
 */
void LoadPairVectors(std::string fileName, std::vector<std::vector<std::pair<std::string, Tensor<float>>>> &output)
{
    std::vector<std::pair<std::string, Tensor<float>>> _sampleVector;
    ...
    for (std::size_t _s = 0; _s < doc.size(); ++_s)
    {
        ...
        _sampleVector.push_back(std::make_pair(_label, _fusedSample));
    }
    output.push_back(_sampleVector);
}
 ...
 *
 * @param fileName the save location of the csv file that contains the data.
 * @param label the label of the class responsible for these weights
...
/**
 * @brief This function saves the filter that has been updated, this can shed a light on which filter has been updated how many times.
 *
 * @param fileName the save location of the txt file that contains the info.
 */
void LogUpdatedFilter(std::string fileName, size_t filter_number)
{
    std::ofstream _File;
    _File.open(fileName + ".txt", std::ios_base::app);
    _File << filter_number << "\n";
    _File.close();
}
...
/**
 * @brief This function saves the number of spikes per layer.
 *
 * @param fileName the save location of the txt file that contains the number of spikes.
 */
void LogSpikeNumber(std::string fileName, size_t spike_number)
{
    std::ofstream _File;
    _File.open(fileName + ".txt", std::ios_base::app);
    _File << std::to_string(spike_number) + "\n";
    _File.close();
}
                        ...
                        std::vector<std::pair<std::string, Tensor<float>>> &_fuse, std::string _draw_fused_path)
{
    ...
    size_t _conv_depth = std::min(_space[0].second.shape().dim(3), _time[0].second.shape().dim(3));

    if (_space[0].second.shape().dim(3) > _time[0].second.shape().dim(3))
        _space = temporal_pooling(_space, 1);
    else if (_space[0].second.shape().dim(3) < _time[0].second.shape().dim(3))
        _time = temporal_pooling(_time, 1);
    if (_space[0].second.shape().dim(1) > _time[0].second.shape().dim(1))
        _space = spatial_pooling(_space, _time[0].second.shape().dim(1));
    else if (_space[0].second.shape().dim(1) < _time[0].second.shape().dim(1))
        _time = spatial_pooling(_time, _width);
    _height = _space[0].second.shape().dim(0);
    _width = _space[0].second.shape().dim(1);
    ...
    for (std::size_t _s = 0; _s < _size; ++_s)
    {
        ...
        // Tensor<float>::normalize_tensor_by_max(_space[_s].second);
        // Tensor<float>::normalize_tensor_by_max(_time[_s].second);
        // CONV_DEPTH by being incremented every frame.
        for (size_t conv = 0; conv < _conv_depth; conv++)
        ...
        {
            Tensor<float>::draw_tensor(_draw_fused_path + "/space/label:" + _fuse_sample.first + "_space_sample:" + std::to_string(_s), _space[_s].second);
            Tensor<float>::draw_tensor(_draw_fused_path + "/time/label:" + _fuse_sample.first + "_time_sample:" + std::to_string(_s), _time[_s].second);
            Tensor<float>::draw_tensor(_draw_fused_path + "/concat/label:" + _fuse_sample.first + "_concat_sample:" + std::to_string(_s), _fuse_sample.second);
        }
    }
...
std::vector<std::pair<std::string, Tensor<float>>> temporal_pooling(std::vector<std::pair<std::string, Tensor<float>>> &_stream, size_t _new_val)
{
    size_t _height = _stream[0].second.shape().dim(0);
    size_t _width = _stream[0].second.shape().dim(1);
    size_t _depth = _stream[0].second.shape().dim(2);
    size_t _old_val = _stream[0].second.shape().dim(3);

    std::vector<std::pair<std::string, Tensor<float>>> _stream_out;
    Tensor<float> out(Shape({_width, _height, _depth, _new_val}));

    for (const std::pair<std::string, Tensor<float>> _sample : _stream)
    {
        ...
        _stream_out.push_back(std::make_pair(_sample.first, out));
    }

    return _stream_out;
}

std::vector<std::pair<std::string, Tensor<float>>> spatial_pooling(std::vector<std::pair<std::string, Tensor<float>>> &_stream, size_t _new_val)
{
    size_t _old_height = _stream[0].second.shape().dim(0);
    size_t _old_width = _stream[0].second.shape().dim(1);
    size_t _depth = _stream[0].second.shape().dim(2);
    size_t _conv_depth = _stream[0].second.shape().dim(3);
    size_t output_width = std::min<size_t>(_new_val, _old_width);
    size_t output_height = std::min<size_t>(_new_val, _old_height);

    std::vector<std::pair<std::string, Tensor<float>>> _stream_out;
    Tensor<float> out(Shape({output_width, output_height, _depth, _conv_depth}));

    size_t filter_width = _old_width / output_width;
    size_t filter_height = _old_height / output_height;
                ...
                for (size_t z = 0; z < _depth; z++)
                    for (size_t k = 0; k < _conv_depth; k++)
                    {
                        ...
                        for (size_t fx = 0; fx < filter_width; fx++)
                        {
                            ...
                            {
                                v += _sample.second.at(x * filter_width + fx, y * filter_height + fy, z, k);
                            }
                        }
                        out.at(x, y, z, k) = v;
                    }
 ...
 * @brief Concatination in the width dimension.
 */
void FuseStreamsConcat(std::vector<std::vector<std::pair<std::string, Tensor<float>>>> &_streams,
                       std::vector<std::pair<std::string, Tensor<float>>> &_fuse, std::string _draw_fused_path)
{
    size_t _height = _streams[0][0].second.shape().dim(0);
    size_t _width = _streams[0][0].second.shape().dim(1);
    size_t _depth = _streams[0][0].second.shape().dim(2);
    size_t _conv_depth = _streams[0][0].second.shape().dim(3);

    _fuse.empty();
    size_t streams_size = _streams.size();
    size_t _size = std::min(_streams[0].size(), _streams[1].size());
    _size = std::min(_size, _streams[2].size());
    for (std::size_t _i = 0; _i < _size; ++_i)
    {
        std::pair<std::string, Tensor<float>> _fuse_sample(_streams[0][_i].first, Shape(std::vector<size_t>({_height, _width * streams_size, _depth, _conv_depth})));
        for (std::size_t _j = 0; _j < streams_size; ++_j)
        {
            // CONV_DEPTH by being incremented every frame.
            for (size_t conv = 0; conv < _conv_depth; conv++)
                for (size_t k = 0; k < _depth; k++)
                    for (size_t j = 0; j < _width; j++)
                        for (size_t i = 0; i < _height; i++)
                        {
                            _fuse_sample.second.at(i, j + _j * _width, k, conv) = _streams[_j][_i].second.at(i, j, k, conv);
                        }
        }
        _fuse.push_back(_fuse_sample);
        if ((!_draw_fused_path.empty()) && _i < 100) // only draw the first 100 fused featuremaps
        {
...
void FuseStreamsConcat2(std::vector<std::pair<std::string, Tensor<float>>> &_space,
                        ...
                        std::vector<std::pair<std::string, Tensor<float>>> &_fuse, std::string _draw_fused_path)
{
    size_t _height = std::min(_space[0].second.shape().dim(0), _time[0].second.shape().dim(0));     //_space[0].second.shape().dim(0);
    size_t _width = std::min(_space[0].second.shape().dim(1), _time[0].second.shape().dim(1));      //_space[0].second.shape().dim(1);
    size_t _depth = std::min(_space[0].second.shape().dim(2), _time[0].second.shape().dim(2));      //_space[0].second.shape().dim(2);
    size_t _conv_depth = std::min(_space[0].second.shape().dim(3), _time[0].second.shape().dim(3)); // _space[0].second.shape().dim(3);
 ...
 * @brief Concatination.
 */
void FuseStreamsConcat7(std::vector<std::pair<std::string, Tensor<float>>> &_space,
                        std::vector<std::pair<std::string, Tensor<float>>> &_time,
                        std::vector<std::pair<std::string, Tensor<float>>> &_fuse, std::string _draw_fused_path)
{
    size_t _height = _space[0].second.shape().dim(0);
    ...
    for (std::size_t _s = 0; _s < _size; ++_s)
    {
        std::pair<std::string, Tensor<float>> _fuse_sample(_space[_s].first, Shape(std::vector<size_t>({_height * 2, _width, _depth, _conv_depth})));
        ...
        for (size_t i = 0; i < _height; i++)
            for (size_t conv = 0; conv < _conv_depth; conv++)
                for (size_t k = 0; k < _depth; k++)
                    for (size_t j = 0; j < _width; j++)
                        _fuse_sample.second.at(i, j, k, conv) = _space[_s].second.at(i, j, k, conv);

        for (size_t i = 0; i + _height < _height * 2; i++)
            for (size_t conv = 0; conv < _conv_depth; conv++)
                for (size_t k = 0; k < _depth; k++)
                    for (size_t j = 0; j < _width; j++)
                        _fuse_sample.second.at(i, j, k, conv) = _time[_s].second.at(i, j, k, conv);
