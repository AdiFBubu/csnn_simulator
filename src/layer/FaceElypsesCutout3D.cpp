...
static RegisterClassParameter<FaceElypsesCutout3D, LayerFactory> _register("FaceElypsesCutout3D");
/**
 * FaceElypsesCutout3D is a type of filter applied on an input immage,
 ...
 */
FaceElypsesCutout3D::FaceElypsesCutout3D() : Layer4D(_register),
...
Shape FaceElypsesCutout3D::compute_shape(const Shape &previous_shape)
{
 ...
 */
size_t FaceElypsesCutout3D::train_pass_number() const
{
...
void FaceElypsesCutout3D::process_train_sample(const std::string &label, Tensor<float> &sample, size_t current_pass, size_t current_index, size_t number)
{
	// The training
	if (current_index == 0)
	{
...
void FaceElypsesCutout3D::process_test_sample(const std::string &label, Tensor<float> &sample, size_t current_index, size_t number)
{
	if (current_index == 0)
	{
...
void FaceElypsesCutout3D::train(const std::string &label, const std::vector<Spike> &input_spike, const Tensor<Time> &input_time, std::vector<Spike> &output_spike)
{
	_impl.train(label, input_spike, input_time, output_spike);
}

void FaceElypsesCutout3D::test(const std::string &, const std::vector<Spike> &input_spike, const Tensor<Time> &input_time, std::vector<Spike> &output_spike)
{
	_impl.test(input_spike, input_time, output_spike);
}

void FaceElypsesCutout3D::on_epoch_end()
{
	_lr_th *= _annealing;
...
Tensor<float> FaceElypsesCutout3D::reconstruct(const Tensor<float> &t) const
{
	size_t ki = 1;
...
Tensor<float> FaceElypsesCutout3D::construct_features(const Tensor<float> &t) const
{
	size_t output_width = t.shape().dim(0);
	size_t output_height = t.shape().dim(1);
	size_t output_depth = t.shape().dim(2);
...
#ifdef ENABLE_QT
void FaceElypsesCutout3D::plot_threshold(bool only_in_train)
{
	add_plot<plot::Threshold>(only_in_train, _th);
}

void FaceElypsesCutout3D::plot_evolution(bool only_in_train)
{
...
_priv::FaceElypsesCutout3DImpl::FaceElypsesCutout3DImpl(FaceElypsesCutout3D &model) : _model(model), _a(), _inh(), _wta()
{
}

void _priv::FaceElypsesCutout3DImpl::resize()
{
...
void _priv::FaceElypsesCutout3DImpl::train(const std::vector<Spike> &input_spike, const Tensor<Time> &input_time, std::vector<Spike> &)
{
...
void _priv::FaceElypsesCutout3DImpl::test(const std::vector<Spike> &input_spike, const Tensor<Time> &, std::vector<Spike> &output_spike)
{
	size_t depth = _model.depth();
	Tensor<float> &w = _model._w;
	Tensor<float> &th = _model._th;
...
#else
_priv::FaceElypsesCutout3DImpl::FaceElypsesCutout3DImpl(FaceElypsesCutout3D &model) : _model(model), _a(), _inh()
{
}
...
void _priv::FaceElypsesCutout3DImpl::train(const std::string &label, const std::vector<Spike> &input_spike, const Tensor<Time> &input_time,
									 std::vector<Spike> &output_spike)
{
	this->_label = label;
...
void _priv::FaceElypsesCutout3DImpl::train(const std::vector<Spike> &input_spike, const Tensor<Time> &input_time, std::vector<Spike> &output_spike)
{
